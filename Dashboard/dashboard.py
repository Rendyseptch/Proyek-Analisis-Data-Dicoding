...
day_df['Kondisi Cuaca'] = day_df['Kondisi Cuaca'].map({
...
min_date = pd.to_datetime(day_df['dateday']).min()
max_date = pd.to_datetime(day_df['dateday']).max()

with st.sidebar:
    st.image('https://i2.wp.com/madran.net/wp-content/uploads/2016/06/bike_icon.png?w=1001&ssl=1')
...
# judul
st.header('Rendy Septian Pradana Bike')
...
# jumlah penyewaan berdasarkan musimnya
...
colors1=["#6495ed", "tab:orange"]
colors2=["#6495ed", "tab:orange"]
colors3=["#6495ed", "tab:orange", "tab:green", "tab:red", "tab:purple", "tab:brown", "tab:pink"]
...
st.caption('Copyright @ Rendy Septian Pradana 2024')
